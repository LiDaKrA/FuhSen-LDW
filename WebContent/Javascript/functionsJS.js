...
$(document).ready(function() {
	...
	$( ".searchLidakra" ).click(function() {
		event.preventDefault(); 
		$(".search_div").hide();
		$(".loading_container").show();
		$(".SearchTextBoxHeader").val($(".SearchTextBox").val());
		GetLidakraInformationAJAX($(".SearchTextBox").val());
		
		
		});
	$( ".searchLidakraHeader" ).click(function() {
		
		event.preventDefault(); 
		$(".DataDiv").hide();
		$(".LidakraDiv").hide();
		$(".resultsLidakraDivMenu1").html("");
		$(".resultsLidakraDivMenu2").html("");
		$(".loading_container").show();
		ChangeTABresultsLidakra("resultsLidakraInfoTableTD1");
		GetLidakraInformationAJAX($(".SearchTextBoxHeader").val());
...
function GetLidakraInformationAJAX(SearchWord){
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: JSON.stringify({  GlobalFunction: "GetLidakraInformationAJAX", SearchWord: SearchWord}),
        ...
        success: function (data) {
        	...
        	$("#PeopleCount").val(ResponseJSON["PeopleCount"]);
        	$("#ItemsCount").val(ResponseJSON["ItemsCount"]);
        	$( ".resultsLidakraDivMenu1" ).append(ResponseJSON["googleplus"]);
        	$( ".resultsLidakraDivMenu2" ).append(ResponseJSON["eBay"]);
        	$(".HeaderDiv").show();
    		$(".DataDiv").show();
    		ChangeTABresultsLidakra("resultsLidakraInfoTableTD1");
...
function getprofile(event,Object){
	event.preventDefault(); 
	$(".DataDiv").hide();
...
function ChangeTABresultsLidakra(ID_TAB){
	var TD= "resultsLidakraInfoTableTD";
	var DIV= "resultsLidakraDivMenu";
	for(var i=1;i<=2;i++){
		$("."+DIV+i).hide();
		$("#"+TD+i).css("color","black");
	}
	$("."+DIV+ID_TAB.replace(TD,"")).show();
	$("#"+ID_TAB).css("color","#F1784B");
	if(ID_TAB=="resultsLidakraInfoTableTD1"){
		$("#resultsLidakraInfoTableNum").text($("#PeopleCount").val()+" Results");
		$(".FiltersPeople").show();
		$(".FiltersItems").hide();
		
	}else
	if(ID_TAB=="resultsLidakraInfoTableTD2"){
		$("#resultsLidakraInfoTableNum").text($("#ItemsCount").val()+" Results");	
		$(".FiltersItems").show();
		$(".FiltersPeople").hide();
	}
...
function  AJAXgetProfile(ProfileId){
	 ...
	 $.ajax({
	        ...
	        success: function (data) {
	        	...
	        	$(".ProfileDownDivMenu6").append(ResponseJSON["DBpedia"]);
